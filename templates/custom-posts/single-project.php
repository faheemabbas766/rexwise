...
<figure>
	...
	<?php if ( class_exists( 'WPbraine_Resizer' ) ) : ?>

		<?php echo wp_kses( $img_obj->WPbraine_resize( wp_get_attachment_url( get_post_thumbnail_id( get_the_ID() ), 'full' ), 1100, 600, true ), $allowed_html ); ?>
...
<div class="blog-detail-meta">
		...
		<?php if ( $options->get( 'projects_single_categories' ) ) : ?>
			...
			<div class="pro-cat">
				...
				<span class="pro-title"><?php esc_html_e('Posted In:', 'braine' ); ?></span>
				...
				<?php $lists = get_the_terms( get_the_ID(), 'project_cat' ); ?>
					...
					<?php foreach( $lists as $list ) { 
						...
						echo wp_kses(braine_set( $list,'name' ), true).', ';
		...
		<div class="opening-hours">
			...
			<ul>
				...
				<li> 
					...
					<?php echo ( braine_set( $entry, 'opening_day') ) ? '<span>'.braine_set( $entry, 'opening_day') .':</span>' : ''; ?>



					<?php echo ( braine_set( $entry, 'opening_time') ) ? '<p>'.braine_set( $entry, 'opening_time') .'</p>' : ''; ?>
				...
				<?php if (  $closed = get_post_meta( get_the_ID(), 'project_closed_day', true ) ) : ?>
					...
					<li>
						...
						<i><?php esc_html_e( 'closed', 'braine' ); ?></i>
		...
		<?php if ( $options->get( 'projectsingle_post_share' ) && $options->get( 'projectsingle_social_share' ) ) : ?>
			...
			<div class="sharing">
				...
				<span><?php esc_html_e( 'Social Sharing:', 'braine' ); ?></span>
				...
				<ul class="share-social">
					...
					<?php if ( ! empty( $options->get( 'projectsingle_social_share' ) ) ) : ?>
						...
						foreach ( $options->get( 'projectsingle_social_share' ) as $k => $v) {
							...
							<?php do_action('braine_social_share_output', $k); ?>
