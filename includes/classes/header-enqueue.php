...
class Header_Enqueue {
	...
	public static function scripts() {
		...
		$options = get_theme_mod( 'braine' . '_options-mods' );
		...
		$scripts = apply_filters( 'BRAINE/includes/classes/header_enqueue/scripts', $scripts );
		...
		$header_data = array(
			...
			'nonce'   => wp_create_nonce( BRAINE_NONCE ),

		);



		wp_localize_script( 'jquery', 'braine_data', $header_data );



		if ( braine_set( $options, 'footer_js' ) ) {



			wp_add_inline_script( 'jquery', braine_set( $options, 'footer_js' ) );
	...
	public static function styles() {
		...
		$styles = apply_filters( 'BRAINE/includes/classes/header_enqueue/styles', $styles );
		...
		foreach ( $styles as $name => $style ) {
			...
			if ( strstr( $style, 'http' ) || strstr( $style, 'https' ) || strstr( $style, 'fonts.googleapis' ) ) {
				...
				wp_enqueue_style( "braine-{$name}", $style );

			} else {

				wp_enqueue_style( "braine-{$name}", get_template_directory_uri() . '/' . $style );
		...
		if ( $custom_font = $options->get('theme_custom_font') ) {
            ...
            $header_styles .= braine_custom_fonts_load( $custom_font );

        }



		wp_add_inline_style( 'braine-main-style', $header_styles );
	...
	public static function resource_hints( $urls, $relation_type ) {
		...
		if ( wp_style_is( 'braine-fonts', 'queue' ) && 'preconnect' === $relation_type ) {
	...
	public static function header_styles() {
		...
		$data = \BRAINE\Includes\Classes\Common::instance()->data( 'blog' )->get();
		...
		$settings = get_theme_mod( 'braine' . '_options-mods' );



		if ( $custom_font = braine_set( $settings, 'theme_custom_font' ) ) {



			$styles .= apply_filters('braine_redux_custom_fonts_load', $custom_font );
